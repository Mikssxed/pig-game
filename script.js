...
const displayDiceRoll = () => {
  ...
  if (actualNumber != 1) {
    ...
    actualPlayer.textContent = currentScore;
  } else {
    currentScore = 0;
    actualPlayer.textContent = 0;
    switchPlayer();
  }
};
...
holdScore.addEventListener('click', function () {
  ...
  actualPlayerScore.textContent =
    Number(actualPlayerScore.textContent) +
    Number(actualPlayerCurrentScore.textContent);
  actualPlayerCurrentScore.textContent = 0;
  if (actualPlayerScore.textContent >= 100) {
...
newGame.addEventListener('click', function () {
  currentScore = 0;
  document.querySelectorAll('.score').forEach(item => (item.innerHTML = 0));
  document
    .querySelectorAll('.current-score')
    .forEach(item => (item.innerHTML = 0));
